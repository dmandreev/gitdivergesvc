...
import type { FetchResponse, ProgressEventPayload } from '../api/client'
import { fetchStream } from '../api/client'

export type FetchStreamStatus = 'idle' | 'connecting' | 'streaming' | 'complete' | 'error'

export interface FetchStreamState {
  status: FetchStreamStatus
...
export function useFetchStream(accessToken: string | null) {
  const [state, setState] = useState<FetchStreamState>({ status: 'idle' })
  ...
  const start = useCallback(
    async (repoGuid: string, branches: string[]) => {
      ...
      try {
        for await (const payload of fetchStream(repoGuid, branches, accessToken, controller.signal)) {
